...
#					timer_current = time.time() - timer_start
					...
					gray = cv2.cvtColor(video_frame, cv2.COLOR_BGR2BGRA)
					cv2.imshow('WebCam View', gray)
					cv2.waitKey(1)
			else:
				break
...
def stop_AVrecording(filename,folder=None):
	if folder is None:
		audio_thread.stop()
		video_thread.stop()
		return
...
def file_manager(filename,folder,value = True):
